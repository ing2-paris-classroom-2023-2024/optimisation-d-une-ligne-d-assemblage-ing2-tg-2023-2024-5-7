...
OperationsGraph operationsGraph;
...
void createOperationsGraph(OperationsGraph *graph) {
    graph->nbOperations = 0;
    for (int i = 0; i < MAX_OPERATIONS; i++) {
        for (int j = 0; j < MAX_OPERATIONS; j++) {
            graph->precedences[i][j] = 0;
            graph->exclusions[i][j] = 0;
        }
    }
}

void addOperation(OperationsGraph *graph, int operation) {
    if (graph->nbOperations < MAX_OPERATIONS) {
        graph->operations[graph->nbOperations++] = operation;
    }
}

void computeOperationSequence(OperationsGraph *graph, OperationSequence *sequence) {
    for (int i = 0; i < graph->nbOperations; i++) {
        sequence->operations[i] = graph->operations[i];
        sequence->nbPredecesseurs[i] = 0;
        sequence->temps[i] = 0.0f;
    }

    for (int i = 0; i < graph->nbOperations; i++) {
        for (int j = 0; j < graph->nbOperations; j++) {
            if (graph->precedences[i][j] == 1) {
                sequence->nbPredecesseurs[j]++;
            }
        }
    }
}

void initialiserMatrices(OperationsGraph *graph) {
    for (int i = 0; i < MAX_OPERATIONS; ++i) {
        for (int j = 0; j < MAX_OPERATIONS; ++j) {
            graph->precedences[i][j] = 0;
            graph->exclusions[i][j] = 0;
...
void lireContrainteExclusion(OperationsGraph *graph) {
   ...
   //                      Code des fontcions avec des erreurs d'appel
...
int main() {
    ...
    FILE *fichierCycle = fopen("temps_de_cycle.txt", "r");
